...
transformed data {
  matrix[nSubjects, 4] load_std = load/sd(load);
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:4) {
        ...
        target += normal_lpdf(load_std[s, t] | a[s] * exp(b[s] * reps_std[s, t]), sigma);
...
generated quantities {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:4) {
        ...
        // log likelihood
        log_lik[s] += normal_lpdf(load_std[s, t] | a[s] * exp(b[s] * reps_std[s, t]), sigma);
        
        load_pred[s, t] = a_rec[s] * exp(b_rec[s] * reps[s, t]);
