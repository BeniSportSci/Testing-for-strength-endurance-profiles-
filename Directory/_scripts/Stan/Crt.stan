...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:4) {
        ...
        target += normal_lpdf(load_std[s, t] | a[s] / (reps_std[s, t] - b[s]) + c[s], sigma);
...
generated quantities {
  ...
  alpha_rec = alpha * sd(load) * sd(reps);
  beta_rec = beta * sd(reps) + mean(reps); 
  ...
  for (s in 1:nSubjects) {
    ...
    a_rec[s] = a[s] * sd(load) * sd(reps);
    b_rec[s] = b[s] * sd(reps) + mean(reps);
    c_rec[s] = c[s] * sd(load) + mean(load);
    
    for (t in 1:4) {
        
        // log likelihood
        log_lik[s] += normal_lpdf(load_std[s, t] | a[s] / (reps_std[s, t] - b[s]) + c[s], sigma);
        
        load_pred[s, t] = a_rec[s] / (reps[s, t] - b_rec[s]) + c_rec[s];
